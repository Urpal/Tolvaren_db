...
for gameName, game in gameday_dict[closest_matchday]["games"].items():
    ...
    if 'odds' in game.keys():
        ...
        if (len(game['odds']) != 0):
            h_mean = h_acc/len(game['odds'])
            h_imp = (1/h_mean)*100
            u_mean = u_acc/len(game['odds'])
            u_imp = (1/u_mean)*100
            b_mean = b_acc/len(game['odds'])
            b_imp = (1/b_mean)*100

            h_mean_old = h_acc_old/len(game['odds_original'])
            h_diff = h_mean - h_mean_old
            u_mean_old = u_acc_old/len(game['odds_original'])
            u_diff = u_mean - u_mean_old
            b_mean_old = b_acc_old/len(game['odds_original'])
